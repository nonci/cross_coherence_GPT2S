...
PointNet2 networks, from https://github.com/CVLAB-Unibo/pycarus/blob/main/pycarus/learning/models/pointnet2.py
but without using Open3D
...
class PointNet2EncoderMSG(Module):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor, Union[Tensor, None]]:
        ...
        # return here for local features only
        return feats_l2, xyz_l2, trans_pts
        
        xyz_l3, feats_l3 = self.sa_3(xyz_l2, feats_l2)
        global_feature = feats_l3.squeeze(1).reshape(-1, self.size_global_feature)

        # return here for global features only
        return global_feature, xyz_l3, trans_pts
    
        # return here for global and local features
        return (global_feature, feats_l2), xyz_l3, trans_pts
