...
def preprocess_geometry(game_file, specific_sn_models=None, t2s_data=False):
    ...
    sn_model_to_integer = dict(zip(existing_geo, range(len(existing_geo)))) # dict with <model_id>: <int_idx>
...
def preprocess_language(game_data, spell_corrector=None, replace_rare=0, tokenizer='naive',
                        do_compar_superlative=False, merge_diags=True, t2s_data=False):
    # Lower-case all text.
    game_data['text'] = game_data['text'].apply(str.lower)
    # Merge dialogues.
    if merge_diags and not t2s_data:
        game_data = group_dialogues(game_data, dialogue_symbol)    
    # Tokenize sentences to words.
    game_data = tokenize_game_text(game_data, tokenizer, t2s_data)
    # Spell check.
...
def tokenize_game_text(game_data, tokenizer, t2s_data=False):
